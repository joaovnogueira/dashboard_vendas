...
vendas = pd.DataFrame(dados.groupby('Local da compra')['Preço'].agg(['sum', 'count']))
...
with aba2:
    ...
    with coluna1:
        ...
        qtd_vendas = st.number_input('Quantidade de vendas', 2,10,5)
        fig_vendas = px.bar(vendas[['count']].sort_values('count', ascending = False).head(qtd_vendas),
                                        x= 'count',
                                        y = vendas[['count']].sort_values('count', ascending = False).head(qtd_vendas).index,
                                        text_auto = True,
                                        title = f'Top {qtd_vendas} quantidade de vendas')
        
        fig_vendas.update_layout(yaxis_title = 'Estados')
        fig_vendas.update_layout(xaxis_title = 'Quantidade de vendas')
